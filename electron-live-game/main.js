/**
 * League Live Client → NestJS relay (Electron main process).
 *
 * Merge into your app’s main process:
 *   const { registerLiveGameIpc } = require('./electron-live-game/main');
 *   const { ipcMain } = require('electron');
 *   registerLiveGameIpc(ipcMain);
 *
 * From renderer when gameflow phase changes (Rift/Conduit):
 *   ipcRenderer.send('start-live-polling');
 *   ipcRenderer.send('stop-live-polling');
 *
 * Env: LIVE_GAME_NEST_URL (default http://127.0.0.1:3000)
 */
'use strict';
...
let isPolling = false;
/** True after at least one successful allgamedata fetch this session. */
let hadSuccessfulFetch = false;
/** First successful tick: seed event IDs without replaying history. */
