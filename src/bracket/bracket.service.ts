...
export class BracketService {
  ...
  async generate(dto: CreateBracketDto): Promise<Bracket> {
    ...
    if (existing)
      throw new ConflictException(
        'A bracket already exists for this season/stage.',
      );
    ...
    const totalRounds =
      dto.format === BracketFormat.DOUBLE_ELIMINATION
  ...
  private buildSingleElimination(
    teams: string[],
    totalRounds: number,
    slots: BracketSlot[],
  ): void {
    ...
    for (let pos = 1; pos <= r1Matches; pos++) {
      ...
      slots.push({
        ...
        status: isBye
          ...
          : t1 && t2
            ? BracketSlotStatus.READY
            : BracketSlotStatus.PENDING,
      } as BracketSlot);
    }
    ...
    for (let round = 2; round <= totalRounds; round++) {
      ...
      for (let pos = 1; pos <= matchesInRound; pos++) {
        const nextPos = Math.ceil(pos / 2);
        const nextSlotId =
          round < totalRounds ? `R${round + 1}S${nextPos}` : undefined;
        slots.push({
   ...
   *   1 Grand Final
   */
  private buildDoubleElimination(
    teams: string[],
    ubRounds: number,
    slots: BracketSlot[],
  ): void {
    ...
    for (let pos = 1; pos <= r1Matches; pos++) {
      ...
      const loserNextSlotId =
        lbTotalRounds > 0 ? `LB-R1M${lbR1Pos}` : undefined;
    ...
    if (ubRounds >= 2) {
      slots.push({
        ...
        nextSlotId: 'GF',
        loserNextSlotId:
          lbTotalRounds > 0 ? `LB-R${lbTotalRounds}M1` : undefined,
        status: BracketSlotStatus.PENDING,
      } as BracketSlot);
    }
  ...
  async advanceWinner(seasonId: string, dto: AdvanceSlotDto): Promise<Bracket> {
    ...
    const isFinished = gfSlot
      ...
      : bracket.slots.every(
          (s) =>
            s.status === BracketSlotStatus.COMPLETED ||
            s.status === BracketSlotStatus.BYE,
        );
