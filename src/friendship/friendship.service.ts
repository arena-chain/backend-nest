...
export class FriendshipService {
  ...
  async getFriends(userId: string): Promise<any[]> {
    const friends = await this.friendshipModel
      ...
      .populate('requesterId', 'nickname email avatar role')
      .populate('recipientId', 'nickname email avatar role')
      .exec();

    // Filter out friends who are admins or system users
    return friends.filter((f: any) => {
      const requester = f.requesterId;
      const recipient = f.recipientId;
      const otherUser = requester._id.toString() === userId ? recipient : requester;
      
      const role = (otherUser.role || '').toLowerCase();
      const nickname = (otherUser.nickname || '').toLowerCase();
  ...
  async areFriends(userId1: string, userId2: string): Promise<boolean> {
    const friendship = await this.friendshipModel.findOne({
      ...
      status: FriendshipStatus.ACCEPTED,
    }).populate('requesterId', 'role').populate('recipientId', 'role').exec();

    if (!friendship) return false;

    // Even if they are friends in DB, block if one is an admin
    const reqRole = (friendship.requesterId as any).role;
    const recRole = (friendship.recipientId as any).role;
    
    return reqRole !== 'admin' && recRole !== 'admin';
  }
