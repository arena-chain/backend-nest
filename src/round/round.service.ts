...
export class RoundService {
  ...
  async generateRounds(dto: GenerateRoundsDto): Promise<Round[]> {
    ...
    if (generateMatches) {
      ...
      const activeTeams = teams.filter(
        (t) => t.status === SeasonTeamStatus.ACTIVE,
      );
    ...
    for (let i = 0; i < weekCount; i++) {
      ...
      if (generateMatches) {
        ...
        if (isSwiss) {
          ...
          pairings = await this.computeSwissPairings(
            seasonId,
            stageId,
            swissRound,
          );
  ...
  private async computeSwissPairings(
    ...
    const standings = await this.standingsService.findBySeason(
      seasonId,
      stageId,
    );
