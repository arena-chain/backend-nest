...
import { AssetPreset, AssetPresetDocument } from './schemas/asset-preset.schema';
...
export class AssetPresetService {
    constructor(@InjectModel(AssetPreset.name) private presetModel: Model<AssetPresetDocument>) {}

    async create(userId: string, dto: CreateAssetPresetDto): Promise<AssetPresetDocument> {
    ...
    async findOne(userId: string, id: string): Promise<AssetPresetDocument> {
        ...
        if (doc.userId.toString() !== userId) throw new ForbiddenException('Not your preset');
        return doc;
    }

    async update(userId: string, id: string, dto: UpdateAssetPresetDto): Promise<AssetPresetDocument> {
        await this.findOne(userId, id);
        const update: any = { ...dto };
        if (dto.baseNftId !== undefined) {
            update.baseNftId = dto.baseNftId ? new Types.ObjectId(dto.baseNftId) : null;
        }
        const doc = await this.presetModel.findByIdAndUpdate(id, update, { new: true }).exec();
        if (!doc) throw new NotFoundException('Preset not found');
        return doc;
    }
