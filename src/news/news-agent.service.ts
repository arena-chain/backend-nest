...
export class NewsAgentService implements OnModuleInit {
  ...
  private readonly browserLikeHeaders = {
    'User-Agent':
  ...
  async onModuleInit() {
    this.logger.log(
      'NewsAgentService initialized. Triggering initial fetch...',
    );
    ...
    try {
      ...
      for (const article of allArticles) {
        try {
          ...
          const exists = await this.newsService.existsBySourceUrl(
            article.sourceUrl,
          );
          if (exists) {
            duplicateCount++;
          ...
          this.logger.error(
            `Failed to publish article: ${article.title}`,
            err.stack,
          );
        }
      }
      ...
      this.logger.log(
        `Fetch cycle complete: ${publishedCount} published, ${duplicateCount} duplicates skipped.`,
      );
    } catch (error) {
  ...
  private async fetchFromNewsApi(): Promise<CreateNewsDto[]> {
    ...
    if (!apiKey) {
      this.logger.warn(
        'NEWSAPI_KEY not found in config. Skipping NewsAPI fetch.',
      );
      return [];
    }
  ...
  private async fetchFromRss(): Promise<CreateNewsDto[]> {
    ...
    for (const feed of rssFeeds) {
      try {
        ...
        const responseBody = await this.fetchTextWithRetries(
          feed.url,
          'RSS fetch',
        ...
        const parsed = await this.rssParser.parseString(responseBody);
  ...
  private detectCategory(title: string): string {
    ...
    if (t.includes('patch') || t.includes('update') || t.includes('notes'))
      return 'patch_notes';
    if (
      t.includes('tournament') ||
      t.includes('esports') ||
      t.includes('pro league') ||
      t.includes('win')
    )
      return 'esports';
  ...
  private getAxiosStatus(error: any): number | undefined {
  ...
  private async fetchTextWithRetries(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (status === 403) {
          this.logger.warn(
            `${context} blocked with 403 at ${url}. Skipping source.`,
          );
          return null;
        }
        ...
        if (!isFinalAttempt) {
          ...
          this.logger.warn(
            `${context} failed for ${url} (attempt ${attempt}/${maxRetries}, status: ${status ?? 'timeout/network'}). Retrying in ${backoffMs}ms.`,
          );
        ...
        this.logger.error(
          `${context} failed for ${url} after ${maxRetries} attempts (status: ${status ?? 'timeout/network'}).`,
          error.stack,
        );
        return null;
      }
  ...
  private async fetchRapidApiGameNews(
    gameId: string,
    apiKey: string,
  ): Promise<any[]> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        // Invalid key / missing subscription / forbidden endpoint
        if (status === 403) {
          this.logger.warn(
            `RapidAPI returned 403 for game ${gameId}. Check RAPIDAPI_KEY and subscription for games-details API.`,
          );
          return [];
        }
        ...
        if (status === 429 && attempt < maxRetries) {
          ...
          this.logger.warn(
            `RapidAPI rate limit for game ${gameId} (attempt ${attempt}/${maxRetries}). Retrying in ${backoffMs}ms.`,
          );
          await this.sleep(backoffMs);
          continue;
        }
        ...
        this.logger.error(
          `Error fetching from RapidAPI for game ${gameId} (status: ${status ?? 'unknown'})`,
          error.stack,
        );
        return [];
      }
  ...
  private async fetchFromRapidApi(): Promise<CreateNewsDto[]> {
    ...
    if (!apiKey) {
      this.logger.warn(
        'RAPIDAPI_KEY not found in config. Skipping RapidAPI fetch.',
      );
      return [];
    }
    ...
    for (const gameId of gameIds) {
      const articles = await this.fetchRapidApiGameNews(gameId, apiKey);
      for (const article of articles) {
        allArticles.push({
          ...
          sourceUrl:
            article.url || `https://games-details-rapidapi.com/${article.id}`,
      ...
      // Small pacing between game IDs to reduce bursts.
  ...
  private async fetchFromScraping(): Promise<CreateNewsDto[]> {
    ...
    try {
      ...
      if (!responseBody) return articles;
        ...
        if (title && sourceUrl) {
          articles.push({
            ...
            sourceUrl: sourceUrl.startsWith('http')
              ? sourceUrl
              : `https://www.esports.net${sourceUrl}`,
      ...
      $('.entry').each((_, element) => {
        ...
        if (coverImageUrl && coverImageUrl.startsWith('data:image')) {
          coverImageUrl =
            img.attr('data-src') || img.attr('data-lazy-src') || undefined;
        }
      ...
      this.logger.log(
        `Successfully parsed ${articles.length} news items from ${targetUrl}`,
      );
    } catch (error) {
