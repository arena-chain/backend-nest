...
export class RiotApiService {
    ...
    constructor(
        ...
        this.apiKey = apiKey;
        console.log('RiotApiService: API Key loaded successfully (starts with: ' + apiKey.substring(0, 6) + '...)');
    }

    async getPlayerAccount(dto: FetchAccountDto): Promise<PlayerAccountInfo> {
        console.log(`RiotApiService: Fetching account for ${dto.gameName}#${dto.tagLine} in ${dto.region}`);
        try {
            ...
            const allRanks = await this.getLeagueEntriesBySummoner(summoner.id, dto.region);
            const soloRank = allRanks.filter(r => r.queueType === 'RANKED_SOLO_5x5');
            ...
            const matchHistoryResults = await Promise.allSettled(
                matchIds.map(id => this.getMatchDetailsById(id, dto.region, account.puuid))
            );

            const matchHistory = matchHistoryResults
                .filter((res): res is PromiseFulfilledResult<RiotMatchInfo> => res.status === 'fulfilled')
                .map(res => res.value);

            console.log(`RiotApiService: Successfully fetched details for ${matchHistory.length} matches`);
    ...
    async getTftAccount(dto: FetchAccountDto): Promise<any> {
        console.log(`RiotApiService: Fetching TFT data for ${dto.gameName}#${dto.tagLine} in ${dto.region}`);
        try {
            const account = await this.getAccountByRiotId(dto.gameName, dto.tagLine, dto.region);
            const matchIds = await this.getTftMatchIdsByPuuid(account.puuid, dto.region);

            const matchHistoryResults = await Promise.allSettled(
                matchIds.map(id => this.getDetailedTftMatchInfo(id, dto.region, account.puuid))
            );
            ...
            throw new HttpException('Failed to fetch TFT info', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }

    private cleanRiotId(val: string): string {
        if (!val) return val;
        // Strip invisible unicode formatting characters (LRI, PDI, etc.)
    ...
    private async getAccountByRiotId(
        ...
        const cleanName = this.cleanRiotId(gameName);
        const cleanTag = this.cleanRiotId(tagLine);

        const routing = REGION_TO_ROUTING[region];
        const url = `https://${routing}.api.riotgames.com/riot/account/v1/accounts/by-riot-id/${encodeURIComponent(cleanName)}/${encodeURIComponent(cleanTag)}`;
    ...
    private async getMatchDetailsById(
        ...
        try {
            ...
            if (!participant) {
                console.warn(`RiotApiService: Participant with PUUID ${puuid} not found in match ${matchId}`);
            ...
            throw new HttpException(`Failed to fetch match details for ${matchId}`, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    async getDetailedMatchById(
        ...
        try {
            ...
            const formatTeamStats = (team: any, participants: any[]): TeamObjectiveStats => ({
                totalKills: participants.reduce((sum, p) => sum + (p.kills || 0), 0),
                totalGold: participants.reduce((sum, p) => sum + (p.goldEarned || 0), 0),
            ...
            console.error(`Error fetching detailed match for ${matchId}: ${error.message}`);
            if (error instanceof HttpException) throw error;
            throw new HttpException(`Failed to fetch match details`, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    async getDetailedTftMatchInfo(
        ...
        try {
            ...
            console.error(`Error fetching detailed TFT match for ${matchId}: ${error.message}`);
            if (error instanceof HttpException) throw error;
    ...
    async linkAccount(userId: string, dto: LinkAccountDto) {
        const cleanName = this.cleanRiotId(dto.gameName);
        const cleanTag = this.cleanRiotId(dto.tagLine);
        
        const account = await this.getAccountByRiotId(cleanName, cleanTag, dto.region);
        const summoner = await this.getSummonerByPuuid(account.puuid, dto.region);

        await this.playerService.findOrCreateByUserId(userId);
        await this.playerService.update(userId, {
            riotPuuid: account.puuid,
            riotGameName: cleanName,
            riotTagLine: cleanTag,
