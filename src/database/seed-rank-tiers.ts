...
import { RankTierConfig, TierName } from '../rank/schemas/rank-tier-config.schema';
...
async function seedRankTiers() {
    ...
    try {
        const rankTierConfigModel = app.get<Model<RankTierConfig>>('RankTierConfigModel');
        ...
        for (const config of TIER_CONFIGURATIONS) {
            ...
            console.log(`✅ Seeded tier: ${config.tier} (${config.minElo} - ${config.maxElo || '∞'} ELO)`);
        }
