...
export class InventoryController {
  ...
  @ApiOperation({
    ...
    description:
      'Returns the current user inventory with all items and equipped items',
  })
  ...
  @Post('equip')
  @ApiOperation({
    summary: 'Equip an NFT item to a slot',
    description: 'Equip a weapon, avatar, skin, etc. to a specific slot',
  })
  ...
  @Get('equipped')
  @ApiOperation({
    summary: 'Get my equipped items',
    description: 'Returns only the currently equipped items',
  })
  ...
  setWallet(@Req() req, @Param('walletAddress') walletAddress: string) {
    return this.inventoryService.linkExternalWallet(
      req.user.userId,
      walletAddress,
    );
  }
