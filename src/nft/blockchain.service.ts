...
export class BlockchainService implements OnModuleInit {
  ...
  private gtkMetadataCache: {
    value: GameTokenMetadata | null;
    expiresAt: number;
  } | null = null;
  private gtkBalanceCache = new Map<
    string,
    { value: GameTokenBalanceRow; expiresAt: number }
  >();
  ...
  onModuleInit() {
    ...
    const network =
      this.configService.get<string>('ALCHEMY_NETWORK') || 'eth-sepolia';
    const rpcUrlFallback =
      this.configService.get<string>('RPC_URL')?.trim() ||
    ...
    if (apiKey) {
      ...
      this.logger.warn(
        'ALCHEMY_API_KEY not set — Alchemy / NFT helpers will be disabled',
      );
      if (!this.jsonRpcProvider) {
        this.logger.warn(
          'No RPC_URL — GTK JSON-RPC disabled until RPC_URL or ALCHEMY_API_KEY is set',
        );
      }
    }
  ...
  async diagnoseGameToken(): Promise<{
    ...
    try {
      ...
      hints.push(
        'GAME_TOKEN_CONTRACT_ADDRESS is not a valid 0x-prefixed 20-byte address.',
      );
      return {
    ...
    try {
      ...
      hints.push(
        'eth_getCode failed — check RPC_URL and that the node is running.',
      );
      return { code: 'RPC_UNREACHABLE', hints, detail: e?.message };
    }
  ...
  private isLikelyLocalAnvilRpc(): boolean {
    const rpc =
      this.configService.get<string>('RPC_URL')?.trim() ||
      process.env.RPC_URL?.trim() ||
      '';
  ...
  private readGameTokenAddressFromBroadcast(chainId: number): string | null {
    try {
      ...
      const gameTokenTx = txs.find(
        (t) => t.contractName === 'GameToken' && t.contractAddress,
      );
      const fallbackTx = [...txs]
        .reverse()
        .find((t) => !!t.contractAddress);
      const raw =
        gameTokenTx?.contractAddress?.trim() ||
        fallbackTx?.contractAddress?.trim();
      if (!raw) {
        return null;
      }
  ...
  private async tryAutoSyncGameTokenAddressFromBroadcast(
    reason: string,
  ): Promise<boolean> {
    if (!this.jsonRpcProvider || !this.isLikelyLocalAnvilRpc()) {
      return false;
    }
    let network: ethers.providers.Network;
    try {
      network = await this.jsonRpcProvider.getNetwork();
    } catch {
      return false;
    }
    ...
    const fromBroadcast = this.readGameTokenAddressFromBroadcast(network.chainId);
    if (!fromBroadcast) {
      return false;
    }
    if (
      this.gameTokenContractAddress &&
      ethers.utils.getAddress(this.gameTokenContractAddress) === fromBroadcast
    ) {
      return false;
    }
    this.gameTokenContractAddress = fromBroadcast;
    this.invalidateGameTokenMetadataCache();
    this.logger.warn(
      `[GTK] Auto-synced GAME_TOKEN_CONTRACT_ADDRESS from latest broadcast (${reason}): ${fromBroadcast}`,
    );
    return true;
  }
  ...
  getGameTokenEnvFallbackMetadata(): GameTokenMetadata | null {
    ...
    const symbol =
      ...
      'GTK';
    const { displayName, displaySymbol } = this.gameTokenDisplayLabels(
      name,
      symbol,
    );
   ...
   */
  private gameTokenDisplayLabels(
    chainName: string,
    chainSymbol: string,
  ): {
    displayName: string;
    ...
    const isDefaultGameToken =
      chainSymbol.toUpperCase() === 'GTK' &&
      chainName.toLowerCase() === 'gametoken';

    const displayName =
      envName || (isDefaultGameToken ? 'Vex currency' : chainName);
    const displaySymbol =
      envSymbol || (isDefaultGameToken ? 'VEX' : chainSymbol);
  ...
  async getGameTokenMetadata(): Promise<GameTokenMetadata | null> {
    ...
    try {
      const checksummedAddr = ethers.utils.getAddress(this.gameTokenContractAddress!);
      const initialCode = await this.jsonRpcProvider!.getCode(checksummedAddr);
      if (!initialCode || initialCode === '0x') {
        await this.tryAutoSyncGameTokenAddressFromBroadcast('metadata read');
      }
  ...
  async getGameTokenBalance(
    holderAddress: string,
  ): Promise<GameTokenBalanceRow> {
    if (!this.isGameTokenConfigured()) {
      throw new Error(
        'Game token (GTK) is not configured — set GAME_TOKEN_CONTRACT_ADDRESS and RPC',
      );
    }
    ...
    let tokenAddr = ethers.utils.getAddress(this.gameTokenContractAddress!);
    ...
    let code = await provider.getCode(tokenAddr);
    if (!code || code === '0x') {
      const synced = await this.tryAutoSyncGameTokenAddressFromBroadcast(
        'balance read',
      );
      if (synced && this.gameTokenContractAddress) {
        tokenAddr = ethers.utils.getAddress(this.gameTokenContractAddress);
    ...
    try {
      ...
      const method =
        typeof err?.method === 'string' ? err.method : 'ERC-20 view call';
  ...
  getZeroBalanceDisplay(
    decimals: number,
  ): Pick<GameTokenBalanceRow, 'balanceRaw' | 'balanceFormatted'> {
    return {
      balanceRaw: '0',
  ...
  async mintGameToken(
    ...
    if (!this.wallet) {
      throw new Error(
        'Minting is not configured — set WALLET_PRIVATE_KEY or PRIVATE_KEY in .env',
      );
    }
    if (!this.isGameTokenConfigured()) {
    ...
    this.logger.log(
      `GameToken mint — tx: ${receipt.transactionHash}, to: ${checksum}`,
    );
  ...
  async verifyGameTokenBurnTx(
    ...
    if (total.isZero()) {
      throw new Error(
        'No burn (Transfer to address(0)) from your wallet found in this transaction',
      );
    }
  ...
  async mintNft(params: {
    ...
    if (!this.wallet) {
      throw new Error(
        'Minting is not configured — set WALLET_PRIVATE_KEY in .env',
      );
    }
    ...
    try {
      ...
      this.logger.log(
        `NFT minted — tx: ${receipt.transactionHash}, tokenId: ${tokenId}`,
      );
  ...
  async transferNft(params: {
    ...
    if (!this.wallet) {
      throw new Error(
        'Transfer is not configured — set WALLET_PRIVATE_KEY in .env',
      );
    }
