...
import {
  NftAttribute,
  NftAttributeDocument,
} from './schemas/nft-attribute.entity';
import { NftItem, NftItemDocument } from './schemas/nft-item.entity';
import {
  NftCollection,
  NftCollectionDocument,
} from './schemas/nft-collection.entity';
import {
  NftTransaction,
  NftTransactionDocument,
} from './schemas/nft-transaction.entity';
...
export class NftService {
  ...
  constructor(
    ...
    @InjectModel(NftAttribute.name)
    private nftAttributeModel: Model<NftAttributeDocument>,
    ...
    @InjectModel(NftTransaction.name)
    private nftTransactionModel: Model<NftTransactionDocument>,
  ...
  async create(creatorId: string, dto: CreateNftDto): Promise<NftDocument> {
    if (dto.collectionId) {
      const collection = await this.nftCollectionModel.findById(
        dto.collectionId,
      );
    ...
    const nft = new this.nftModel({
      ...
      collectionId: dto.collectionId
        ...
        : undefined,
      compatibleGames:
        dto.compatibleGames?.map((id) => new Types.ObjectId(id)) || [],
    });
    return nft.save();
  }
  ...
  async findAll(filters?: {
    ...
    if (filters?.collectionId)
      query.collectionId = new Types.ObjectId(filters.collectionId);
  ...
  async update(id: string, dto: UpdateNftDto): Promise<NftDocument> {
    ...
    if (dto.compatibleGames) {
      updateData.compatibleGames = dto.compatibleGames.map(
  ...
  async addAttribute(
    dto: CreateNftAttributeDto,
  ): Promise<NftAttributeDocument> {
  ...
  async getAttributes(nftId: string): Promise<NftAttributeDocument[]> {
    return this.nftAttributeModel
      .find({ nftId: new Types.ObjectId(nftId) })
      .exec();
  }
  ...
  async mintNft(adminId: string, dto: MintNftDto): Promise<NftItemDocument> {
    ...
    if (nft.status === 'BURNED')
      throw new BadRequestException('Cannot mint a burned NFT');
    ...
    if (this.blockchainService.isMintingEnabled() && contractAddress) {
      ...
      this.logger.warn(
        'Blockchain minting not configured — creating NFT item without on-chain mint',
      );
      nft.status = 'MINTED';
      nft.mintedAt = new Date();
      await nft.save();
  ...
  async transferItem(
    ...
    if (
      ...
      const result = await this.blockchainService.transferNft({
        ...
        fromAddress:
          item.walletAddress || this.blockchainService.getWalletAddress()!,
  ...
  async getStats(): Promise<any> {
    ...
    const totalMinted = await this.nftModel.countDocuments({
      status: 'MINTED',
    });
  ...
  async findAllCollections(filters?: {
    category?: string;
    isActive?: boolean;
  }): Promise<NftCollectionDocument[]> {
    const query: any = {};
    ...
    return this.nftCollectionModel
      .find(query)
      .populate('creatorId', 'nickname email')
      .populate('compatibleGames', 'title genre')
      .exec();
  ...
  async removeCollection(id: string): Promise<void> {
    const nftsInCollection = await this.nftModel.countDocuments({
      collectionId: new Types.ObjectId(id),
    });
    if (nftsInCollection > 0) {
      throw new BadRequestException(
        `Cannot delete collection — it contains ${nftsInCollection} NFTs. Remove or reassign them first.`,
      );
    }
  ...
  async getMarketplaceStats(): Promise<any> {
    const listedItems = await this.nftItemModel
      .find({ status: 'LISTED' })
      .exec();
    ...
    const recentSales = await this.nftTransactionModel.countDocuments({
      type: 'SALE',
    ...
    return {
      ...
      floorPrice:
        listedItems.length > 0
          ? Math.min(...listedItems.map((i) => i.listPrice))
          : 0,
    };
  }
