...
export class NftController {
  ...
  @ApiOperation({
    summary: 'Create a new NFT (Admin only)',
    description: 'Create a new NFT piece — weapon, avatar, skin, etc.',
  })
  ...
  @ApiOperation({
    ...
    description: 'List all NFT pieces with optional filters',
  })
  @ApiQuery({
    name: 'category',
    required: false,
  ...
  @ApiResponse({
    status: 200,
    description: 'List of NFT items owned by the user',
  })
  getMyItems(@Req() req) {
  ...
  transfer(@Req() req, @Body() dto: TransferNftItemDto) {
    return this.nftService.transferItem(
      req.user.userId,
      dto.nftItemId,
      dto.toUserId,
      dto.toWalletAddress,
  ...
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({
    summary: 'Create an NFT collection (Admin only)',
    description: 'Create a collection such as Avatars, Weapons, etc.',
  })
  ...
  updateCollection(
    ...
    @Body() dto: UpdateNftCollectionDto,
  ) {
  ...
  @Roles(UserRole.ADMIN)
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({ summary: 'Toggle featured status of an item' })
  @ApiResponse({ status: 200, description: 'Status toggled' })
  toggleFeature(@Param('id') id: string) {
    return this.nftService.toggleFeatured(id);
  }

  @Post(':id/force-unlist')
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(UserRole.ADMIN)
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({ summary: 'Force unlist an item (Admin only)' })
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({ summary: 'List an NFT item for sale' })
  @ApiParam({ name: 'id', description: 'NFT Item ID' })
  @ApiBody({
    schema: { type: 'object', properties: { listPrice: { type: 'number' } } },
  })
  @ApiResponse({ status: 200, description: 'Item listed for sale' })
  list(
    @Req() req,
    @Param('id') id: string,
    @Body('listPrice') listPrice: number,
  ) {
    return this.nftService.listForSale(req.user.userId, id, listPrice);
  }

  @Post(':id/unlist')
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({ summary: 'Remove an NFT item from sale' })
  @ApiParam({ name: 'id', description: 'NFT Item ID' })
  @ApiResponse({ status: 200, description: 'Item removed from sale' })
  unlist(@Req() req, @Param('id') id: string) {
    return this.nftService.unlist(req.user.userId, id);
  }

  @Post(':id/buy')
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({ summary: 'Buy a listed NFT item' })
  @ApiParam({ name: 'id', description: 'NFT Item ID' })
