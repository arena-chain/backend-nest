...
export class SeasonRule {
  ...
  @Prop({
    required: true,
    enum: TiebreakerRule,
    default: TiebreakerRule.GAME_DIFF,
  })
  ...
  @Prop({
    enum: MapVetoFormat,
    default: MapVetoFormat.BAN_BAN_PICK_PICK_BAN_BAN_DECIDER,
  })
  ...
  @Prop({
    type: [String],
    enum: Object.values(RuleUsage),
    default: [RuleUsage.REGULAR_SEASON],
  })
  ruleUsage: RuleUsage[];
  ...
  @Prop({
    enum: ScoreSubmissionMethod,
    default: ScoreSubmissionMethod.ADMIN_VERIFIED,
  })
  ...
  @Prop({
    type: {
      format: {
        type: String,
        enum: Object.values(OvertimeFormat),
        default: OvertimeFormat.NONE,
      },
