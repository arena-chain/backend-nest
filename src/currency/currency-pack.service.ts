...
import { CurrencyPack, CurrencyPackDocument } from './schemas/currency-pack.schema';
import { CurrencyPackPurchase, CurrencyPackPurchaseDocument } from './schemas/currency-pack-purchase.schema';
...
export class CurrencyPackService {
    constructor(
        @InjectModel(CurrencyPack.name) private readonly packModel: Model<CurrencyPackDocument>,
    ...
    async create(adminUserId: string, dto: CreateCurrencyPackDto): Promise<CurrencyPackDocument> {
    ...
    async update(packId: string, dto: UpdateCurrencyPackDto): Promise<CurrencyPackDocument> {
        ...
        if (dto.grantWholeTokens !== undefined) pack.grantWholeTokens = dto.grantWholeTokens;
        if (dto.priceCents !== undefined) pack.priceCents = dto.priceCents;
        if (dto.priceCurrency !== undefined) pack.priceCurrency = dto.priceCurrency.toUpperCase();
    ...
    async remove(packId: string): Promise<void> {
        ...
        const r = await this.packModel.deleteOne({ _id: new Types.ObjectId(packId) }).exec();
        if (r.deletedCount === 0) {
            throw new NotFoundException('Pack not found');
        }
    }
    ...
    async findAllForAdmin(): Promise<CurrencyPackDocument[]> {
        return this.packModel.find().sort({ sortOrder: 1, createdAt: -1 }).lean().exec() as unknown as CurrencyPackDocument[];
    }
    ...
    async listMyPurchases(userId: string, limit = 50): Promise<CurrencyPackPurchaseDocument[]> {
     ...
     */
    async purchasePack(userId: string, packId: string): Promise<{
        purchaseId: string;
        ...
        if (!this.blockchainService.isMintingEnabled()) {
            throw new BadRequestException('Server minting key not configured (WALLET_PRIVATE_KEY / PRIVATE_KEY).');
        }
        if (!this.blockchainService.isGameTokenConfigured()) {
            throw new BadRequestException('Game token RPC not configured — set GAME_TOKEN_CONTRACT_ADDRESS and RPC_URL.');
        }
        ...
        if (!inventory.walletAddress) {
            throw new BadRequestException('Link a wallet on your inventory before purchasing a pack.');
        }
        ...
        const wei = ethers.utils.parseUnits(String(pack.grantWholeTokens), meta.decimals);
        let transactionHash: string;
        try {
            const r = await this.blockchainService.mintGameToken(inventory.walletAddress, wei);
