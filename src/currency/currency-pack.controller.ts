...
export class CurrencyPackController {
    ...
    async myPurchases(@Req() req: { user: { userId: string } }) {
        const rows = await this.currencyPackService.listMyPurchases(req.user.userId);
        return {
    ...
    async create(@Req() req: { user: { userId: string } }, @Body() dto: CreateCurrencyPackDto) {
    ...
    async purchase(@Req() req: { user: { userId: string } }, @Param('id') id: string) {
