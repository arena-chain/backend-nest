...
import { GiftSimulatedDto } from './dto/gift-simulated.dto';
...
export class CurrencyController {
  ...
  private economyFlags() {
    const testCreditMintAvailable =
      this.isTestCreditMintAllowed() &&
      this.blockchainService.isMintingEnabled();
    const purchaseSimulationAvailable =
      this.isPurchaseSimulationAllowed() &&
      this.blockchainService.isMintingEnabled();
    const economySpendSimulationAvailable =
      this.isEconomySpendSimulationAllowed();
    return {
      testCreditMintAvailable,
      purchaseSimulationAvailable,
      economySpendSimulationAvailable,
    };
  }
  ...
  private async mintWholeToLinkedWallet(userId: string, wholeAmount: number) {
    ...
    const { transactionHash } = await this.blockchainService.mintGameToken(
  ...
  private async safeLedgerIn(params: {
    ...
    try {
      ...
      this.logger.error(
        `Ledger in failed after mint (tx ${params.txHash}): ${e?.message || e}`,
      );
      return false;
    }
  }
  ...
  async getGameTokenConfig() {
    ...
    if (!meta) {
      ...
      throw new ServiceUnavailableException({
        ...
        error: 'Service Unavailable',
        message:
          'Game token contract is present but ERC-20 metadata (name/symbol/decimals) could not be read.',
  ...
  async getMyGameToken(@Req() req: { user: { userId: string } }) {
    ...
    let inventory = await this.inventoryService.getOrCreateInventory(
      req.user.userId,
    );
    if (!inventory.walletAddress && this.isAutoProvisionAppWallet()) {
      await this.inventoryService.ensureAppWallet(req.user.userId);
      inventory = await this.inventoryService.getOrCreateInventory(
        req.user.userId,
      );
    }
    const walletAddress = inventory.walletAddress;
    const branding =
      await this.blockchainService.resolveGameTokenMetadataForApi();
    if (!branding) {
      throw new ServiceUnavailableException(
    ...
    if (!this.blockchainService.isGameTokenConfigured()) {
      const zero = this.blockchainService.getZeroBalanceDisplay(
        branding.decimals,
      );
      return {
        linked: true,
    ...
    try {
      const bal =
        await this.blockchainService.getGameTokenBalance(walletAddress);
      return {
        linked: true,
        walletAddress,
      ...
      this.logger.warn(
        `GET game-token/me: GTK on-chain read failed for ${walletAddress}: ${msg}`,
      );
      const zero = this.blockchainService.getZeroBalanceDisplay(
        branding.decimals,
      );
  ...
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({
    summary: 'Paginated in/out history for your game token wallet',
  })
  ...
  async getGameTokenLedger(
    ...
    const { items, total } = await this.currencyLedgerService.findByUserId(
      req.user.userId,
      p,
      l,
    );
  ...
  async linkMyGameTokenWallet(
    ...
    const inv = await this.inventoryService.linkExternalWallet(
      req.user.userId,
      dto.walletAddress,
    );
    return {
  ...
  async getMyGameTokenWallet(@Req() req: { user: { userId: string } }) {
    ...
    const inventory = await this.inventoryService.getOrCreateInventory(
      req.user.userId,
    );
    const flags = this.economyFlags();
    return {
  ...
  @ApiResponse({
    status: 403,
    description: 'GTK_ALLOW_TEST_CREDIT not enabled',
  })
  async creditTestGameToken(
    @Req() req: { user: { userId: string } },
    @Body() dto: CreditTestGameTokenDto,
  ) {
    if (!this.isTestCreditMintAllowed()) {
      throw new ForbiddenException(
        'Test credit mint is disabled (set GTK_ALLOW_TEST_CREDIT=true to allow).',
      );
    }
    if (!this.blockchainService.isMintingEnabled()) {
      throw new ServiceUnavailableException(
        'Server wallet not configured — set WALLET_PRIVATE_KEY or PRIVATE_KEY',
      );
    }
    if (!this.blockchainService.isGameTokenConfigured()) {
    ...
    try {
      const r = await this.mintWholeToLinkedWallet(
        req.user.userId,
        dto.wholeAmount,
      );
      ...
      throw new BadRequestException(
        e?.message ||
          'Mint failed — is the server key the GameToken contract owner?',
      );
    }
  }
  ...
  @ApiOperation({
    summary:
      'Simulated purchase — mint to linked wallet (GTK_ALLOW_PURCHASE_SIMULATION)',
  })
  async purchaseSimulated(
    @Req() req: { user: { userId: string } },
    @Body() dto: CreditTestGameTokenDto,
  ) {
  ...
  async confirmBurnSell(
    @Req() req: { user: { userId: string } },
    @Body() dto: ConfirmBurnDto,
  ) {
    if (!this.blockchainService.isGameTokenConfigured()) {
      throw new ServiceUnavailableException('Game token is not configured');
    }
    ...
    try {
      const v = await this.blockchainService.verifyGameTokenBurnTx(
        dto.txHash,
        inventory.walletAddress,
      );
      ...
      throw new BadRequestException(
        e?.message || 'Could not verify burn transaction',
      );
    }
  ...
  @ApiOperation({
    summary: 'Record simulated spend (GTK_ALLOW_ECONOMY_SIMULATION)',
  })
  async spendSimulated(
    @Req() req: { user: { userId: string } },
    @Body() dto: SpendSimulatedDto,
  ) {
    ...
    await this.currencyLedgerService.append({
      ...
      decimals: meta.decimals,
      chainId: meta.chainId,
      title: dto.reason?.trim()
        ? `Simulated spend — ${dto.reason.trim()}`
        : 'Simulated spend (in-app)',
      meta: { source: 'spend_simulation', reason: dto.reason ?? null },
    });
    return {
      ok: true,
      recordedWhole: dto.wholeAmount,
      walletAddress: inventory.walletAddress,
    };
  }

  /**
   * Dev/staging only: off-chain simulated p2p gift recorded in ledger for both users.
   * This does not move real on-chain funds; it is for fake-money UX/testing flows.
   */
  @Post('game-token/gift-simulated')
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth('JWT-auth')
  @ApiBody({ type: GiftSimulatedDto })
  @ApiOperation({
    summary: 'Simulate player-to-player gift (GTK_ALLOW_ECONOMY_SIMULATION)',
  })
  async giftSimulated(
    @Req() req: { user: { userId: string } },
    @Body() dto: GiftSimulatedDto,
  ) {
    if (!this.isEconomySpendSimulationAllowed()) {
      throw new ForbiddenException(
        'Simulated gifting is disabled (set GTK_ALLOW_ECONOMY_SIMULATION=true for local demos).',
      );
    }
    const senderUserId = req.user.userId;
    const recipientUserId = dto.recipientUserId;
    if (senderUserId === recipientUserId) {
      throw new BadRequestException('You cannot gift currency to yourself.');
    }
    ...
    const [senderInventory, recipientInventory] = await Promise.all([
      this.inventoryService.getOrCreateInventory(senderUserId),
      this.inventoryService.getOrCreateInventory(recipientUserId),
    ]);

    if (!senderInventory.walletAddress) {
      throw new BadRequestException('Sender wallet is not linked/provisioned.');
    }
    if (!recipientInventory.walletAddress) {
      throw new BadRequestException(
        'Recipient wallet is not linked/provisioned yet.',
      );
    }

    const amountWei = ethers.utils.parseUnits(
      String(dto.wholeAmount),
      meta.decimals,
    );
    const amountFormatted = ethers.utils.formatUnits(amountWei, meta.decimals);
    const giftRef = `gift-sim-${Date.now()}-${Math.floor(Math.random() * 1e6)}`;
    const note = dto.note?.trim();
    const giftTitle = note
      ? `Gift sent — ${note}`
      : 'Gift sent (simulated)';
    const receiveTitle = note
      ? `Gift received — ${note}`
      : 'Gift received (simulated)';

    await this.currencyLedgerService.append({
      userId: senderUserId,
      walletAddress: senderInventory.walletAddress,
      direction: 'out',
      category: 'gift_sent_simulated',
      ...
      title: giftTitle,
      meta: {
        source: 'gift_simulation',
        giftRef,
        toUserId: recipientUserId,
        toWalletAddress: recipientInventory.walletAddress,
      },
    });

    await this.currencyLedgerService.append({
      userId: recipientUserId,
      walletAddress: recipientInventory.walletAddress,
      direction: 'in',
      category: 'gift_received_simulated',
      amountWei: amountWei.toString(),
      amountFormatted,
      decimals: meta.decimals,
      chainId: meta.chainId,
      ...
      meta: {
        ...
        fromWalletAddress: senderInventory.walletAddress,
      },
    });
