...
export class TeamManagerService {
  constructor(
    @InjectModel(TeamManagerProfile.name)
    private teamManagerProfileModel: Model<TeamManagerProfileDocument>,
    @InjectModel(Team.name) private teamModel: Model<TeamDocument>,
    @InjectModel(PlayerProfile.name)
    private playerProfileModel: Model<PlayerProfileDocument>,
  ) {}
  ...
  async findByUserId(
    ...
    const profile = await this.teamManagerProfileModel.findOne({
      userId: userObjectId,
    });
  ...
  async findPending(): Promise<TeamManagerProfileDocument[]> {
    return this.teamManagerProfileModel
      .find({ status: 'pending' })
      .populate('userId')
  ...
  async approve(userId: string): Promise<TeamManagerProfileDocument> {
    ...
    await this.teamModel.findByIdAndUpdate(profile.team, {
  ...
  async createTeamForManager(
    ...
    if (profile.team) {
      throw new ConflictException(
        'Team manager already has a team. Use update instead.',
      );
    }
  ...
  async updateTeam(
    ...
    if (!profile.team) {
      throw new NotFoundException(
        'No team linked to this manager. Create a team first.',
      );
    }
  ...
  async invitePlayer(
    ...
    if (!profile.team) {
      throw new BadRequestException(
        'Create a team first before inviting players',
      );
    }

    const playerObjectId = new Types.ObjectId(playerUserId);

    const playerExists = await this.playerProfileModel.findOne({
