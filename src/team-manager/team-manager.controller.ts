...
export class TeamManagerController {
    ...
    @ApiBearerAuth()
    @ApiOperation({ summary: 'Get my team manager profile (with team populated)' })
    ...
    invitePlayer(@Request() req, @Body() dto: InvitePlayerDto) {
        return this.teamManagerService.invitePlayer(req.user.userId, dto.playerUserId);
    }
    ...
    @UseGuards(JwtAuthGuard)
    @ApiBearerAuth()
    @ApiQuery({ name: 'q', required: false, description: 'Search by nickname or email' })
