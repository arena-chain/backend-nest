...
export class GameAssetsController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Flat list of asset files with URL hints',
  })
  async listFiles() {
    ...
    FileInterceptor('file', {
      storage: diskStorage({
        ...
        filename: (req, file, cb) => {
          ...
          const base =
            (req.body?.name || file.originalname.replace(ext, '') || 'model')
              .toString()
              .trim()
              .toLowerCase()
              .replace(/[^a-z0-9-_]+/g, '-')
  ...
  @ApiResponse({
    status: 201,
    description: 'Model uploaded and ready for frontend use',
  })
  uploadModel(
