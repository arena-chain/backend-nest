...
export class GameAssetsService {
   ...
   */
  async scanInventoryFiles(): Promise<{
    roots: string[];
    ...
    const pairs: {
      abs: string;
      urlPrefix: string;
      source: InventoryFileEntry['source'];
    }[] = [];
    ...
    if (existsSync(uploadsInv)) {
      pairs.push({
        abs: uploadsInv,
        urlPrefix: '/uploads/inventory',
        source: 'uploads/inventory',
      });
    ...
    if (pairs.length === 0) {
      this.logger.warn(
        'No inventory folder found (uploads/inventory or src/inventory)',
      );
      return { roots: [], files: [] };
    }
