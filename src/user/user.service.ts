...
export class UsersService {
    ...
    async findAll(): Promise<UserDocument[]> {
        return this.userModel.find({ 
            role: { $not: /admin|system/i },
            nickname: { $not: /admin|system/i }
        }).select('-passwordHash -emailVerificationOtp -resetPasswordOtp -refreshToken').exec();
    }
    ...
    async searchUsers(query: string, excludeUserId?: string): Promise<UserDocument[]> {
        ...
        const filter: any = {
            ...
            role: { $not: /admin|system/i }, // Hide admins/system users
            nickname: { $not: /admin|system/i },
        };
        ...
        return this.userModel
            .find(filter)
            .select('_id nickname email avatar') // Only return necessary fields
