...
import { SeasonRoster, SeasonRosterDocument, RosterStatus } from './schemas/season-roster.schema';
import { CreateSeasonRosterDto } from './dto/create-season-roster.dto';

@Injectable()
export class SeasonRosterService {
    constructor(
        @InjectModel(SeasonRoster.name) private readonly rosterModel: Model<SeasonRosterDocument>,
    ) {}
    ...
    async create(dto: CreateSeasonRosterDto): Promise<SeasonRoster> {
        ...
        if (existing) throw new ConflictException('Roster already exists for this team in this season.');
        return this.rosterModel.create(dto);
    }

    async addPlayer(seasonId: string, teamId: string, playerId: string): Promise<SeasonRoster> {
        ...
        if (roster.status === RosterStatus.LOCKED) {
            throw new BadRequestException('Roster is locked. Cannot add players after the registration deadline.');
        }
        if (roster.playerIds.length >= roster.maxRosterSize) {
            throw new BadRequestException(`Roster is full. Max roster size is ${roster.maxRosterSize}.`);
        }
    ...
    async removePlayer(seasonId: string, teamId: string, playerId: string): Promise<SeasonRoster> {
        const roster = await this.rosterModel.findOne({ seasonId, teamId });
        if (!roster) throw new NotFoundException('Roster not found.');
        if (roster.status === RosterStatus.LOCKED) {
            throw new BadRequestException('Roster is locked. Cannot remove players after the registration deadline.');
        }
        const pid = new Types.ObjectId(playerId);
    ...
    async lockAllForSeason(seasonId: string): Promise<number> {
        const rosters = await this.rosterModel.find({ seasonId, status: RosterStatus.OPEN });
        let locked = 0;
    ...
    async findByTeamAndSeason(seasonId: string, teamId: string): Promise<SeasonRoster | null> {
        return this.rosterModel
