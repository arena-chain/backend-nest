...
export class LeagueService {
    constructor(
        ...
        @InjectModel(LeagueParticipant.name) private participantModel: Model<LeagueParticipant>,
        private playerService: PlayerService,
    ) { }

    async create(createLeagueDto: CreateLeagueDto, adminId: string): Promise<League> {
    ...
    async findMyLeagues(userId: string): Promise<League[]> {
        const participations = await this.participantModel.find({ playerId: new Types.ObjectId(userId) }).exec();
    ...
    async update(id: string, updateData: any): Promise<League> {
        const league = await this.leagueModel.findByIdAndUpdate(id, updateData, { new: true }).exec();
        if (!league) throw new NotFoundException('League not found');
        return league;
    }
    ...
    async addSupervisors(id: string, moderatorIds: string[]): Promise<League> {
        ...
        league.supervisedBy = [...new Set([...league.supervisedBy.map(s => s.toString()), ...moderatorIds])].map(id => new Types.ObjectId(id));
        return league.save();
    }

    async registerParticipant(leagueId: string, userId: string): Promise<LeagueParticipant> {
        if (!userId || !Types.ObjectId.isValid(userId)) {
            throw new BadRequestException('Your session is invalid or has expired. Please log out and log in again.');
        }
        ...
        if (league.status !== LeagueStatus.UPCOMING) {
            throw new BadRequestException(`Registration for this league is ${league.status.toLowerCase()}.`);
        }
        ...
        try {
            ...
            try {
                ...
                if (createError.code === 11000) {
                    ...
                    throw new BadRequestException('Unable to initialize your player profile. Please contact support.');
                }
            }
        }
        ...
        if (playerElo < requiredElo) {
            throw new BadRequestException(`Minimum ELO required is ${requiredElo}. You have ${playerElo}.`);
        }
        ...
        if (existing) {
            throw new BadRequestException('You are already registered for this league');
        }

        // 4. Check capacity
        const count = await this.participantModel.countDocuments({ leagueId: leagueIdObj });
        if (count >= league.maxParticipants) {
            throw new BadRequestException(`League is full (${league.maxParticipants} participants max)`);
        }
    ...
    async processLeagueRewards(leagueId: string): Promise<void> {
        ...
        for (let i = 0; i < standings.length; i++) {
            ...
            if (rewardCfg && participant.playerId) {
                ...
                const profile = await this.playerService.findByUserId(participant.playerId.toString());
                await this.playerService.update(participant.playerId.toString(), {
                    points: (profile.points || 0) + rewardCfg.points
                });

                // In a real app, you'd also log this prize or send a notification
                console.log(`User ${participant.playerId} awarded ${rewardCfg.prize} and ${rewardCfg.points} points for rank ${rank}`);
            }
        }
     ...
     */
    async processMatchResult(leagueId: string, winnerId: string, loserId: string, isDraw: boolean): Promise<void> {
        const participants = await this.participantModel.find({
            leagueId: new Types.ObjectId(leagueId),
            playerId: { $in: [new Types.ObjectId(winnerId), new Types.ObjectId(loserId)] }
        }).exec();
