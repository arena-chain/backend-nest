...
export class MatchService {
  ...
  async addGameResult(matchId: string, dto: AddGameResultDto): Promise<Match> {
    ...
    const winsNeeded = this.eloService.winsNeededForFormat(
      match.format as 'BO1' | 'BO3' | 'BO5',
    );
    const seriesOver =
      match.team1GamesWon >= winsNeeded || match.team2GamesWon >= winsNeeded;

    if (seriesOver) {
      const winnerId =
        match.team1GamesWon >= winsNeeded ? match.team1Id : match.team2Id;
      const loserId =
        winnerId === match.team1Id ? match.team2Id : match.team1Id;
      const winnerGamesWon =
        winnerId === match.team1Id ? match.team1GamesWon : match.team2GamesWon;
      const loserGamesWon =
        winnerId === match.team1Id ? match.team2GamesWon : match.team1GamesWon;
  ...
  async submitFullResult(
    matchId: string,
    dto: SubmitFullResultDto,
  ): Promise<Match> {
    const match = await this.matchModel.findById(matchId).exec();
    if (!match) throw new NotFoundException(`Match ${matchId} not found`);
    ...
    const winnerId =
      dto.team1GamesWon >= winsNeeded ? match.team1Id : match.team2Id;
    const loserId = winnerId === match.team1Id ? match.team2Id : match.team1Id;
    const winnerGamesWon =
      winnerId === match.team1Id ? dto.team1GamesWon : dto.team2GamesWon;
    const loserGamesWon =
      winnerId === match.team1Id ? dto.team2GamesWon : dto.team1GamesWon;
  ...
  async declareForfeit(
    ...
    if (
      dto.forfeitingTeamId !== match.team1Id &&
      dto.forfeitingTeamId !== match.team2Id
    ) {
      throw new BadRequestException(
        'forfeitingTeamId must be team1Id or team2Id',
      );
    }

    const winnerId =
      dto.forfeitingTeamId === match.team1Id ? match.team2Id : match.team1Id;
  ...
  async findByRound(roundId: string): Promise<Match[]> {
    return this.matchModel
      .find({ roundId })
      .sort({ matchOrder: 1, scheduledStart: 1 })
      .exec();
  }
  ...
  async createScheduledFromPairings(
    ...
    for (let i = 0; i < pairings.length; i++) {
      ...
      const scheduledStart = new Date(
        roundStartDate.getTime() + i * MS_PER_MATCH_SLOT,
      );
  ...
  private async tryAutoAdvanceBracket(match: MatchDocument): Promise<void> {
    ...
    try {
      ...
      const slot = bracket.slots.find(
        (s) =>
          s.team1Id === match.team1Id &&
          s.team2Id === match.team2Id &&
          s.status !== 'COMPLETED' &&
          s.status !== 'BYE',
      );
      if (!slot) return;
