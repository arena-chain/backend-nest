...
export class GamesService {
  ...
  private resolveMissionGameFromCatalogTitle(
    title?: string,
  ): 'lol' | 'valorant' | 'all' {
    if (!title) return 'all';
  ...
  private async emitMissionProgressForCompletedGame(
    game: GameDocument,
  ): Promise<void> {
    const catalog = await this.catalogModel
      .findById(game.game_id)
      .lean()
      .exec();
    ...
    for (const participant of participants) {
      ...
      const withFriends = await this.isUserPlayingWithFriend(
        participant.userId,
        teammateIds,
      );
      const dedupeKey = `game:${game._id.toString()}`;
      await this.missionService.onMatchCompleted(
  ...
  private async updateAndTriggerMissions(
    id: string,
