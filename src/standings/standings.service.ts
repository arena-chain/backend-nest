...
export class StandingsService {
  ...
  async updateAfterMatch(
    ...
    if (stageId) {
      winnerFilter.stageId = stageId;
      loserFilter.stageId = stageId;
    }
    if (groupId) {
      winnerFilter.groupId = groupId;
      loserFilter.groupId = groupId;
    }

    await this.standingsModel
      .findOneAndUpdate(
        winnerFilter,
        {
          $inc: {
  ...
  async updateAfterForfeit(
    ...
    const forfeitingTeam = await this.standingsModel
      .findOneAndUpdate(
        loserFilter,
        {
          $inc: {
