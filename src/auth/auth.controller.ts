...
import { Controller, Post, Body, Get, UseGuards, Req, Res, Patch, BadRequestException } from '@nestjs/common';
...
export class AuthController {
  ...
  async updateProfile(@Req() req, @Body() updateData: { nickname?: string; country?: string; avatar?: string; region?: string }) {
    return this.authService.updateProfile(req.user.email, updateData);
  }
  @Patch('update-password')
  @UseGuards(AuthGuard('jwt'))
  @ApiOperation({ summary: 'Update current user password from settings' })
  ...
  async generateTwoFactorSecret(@Req() req) {
  ...
  async turnOnTwoFactorAuthentication(@Req() req, @Body() body: { token: string }) {
    return this.authService.turnOnTwoFactorAuthentication(req.user.email, body.token);
  }

  @Post('2fa/disable')
  @UseGuards(AuthGuard('jwt'))
  @ApiOperation({ summary: 'Disable 2FA' })
  async disableTwoFactorAuthentication(@Req() req) {
    return this.authService.disableTwoFactorAuthentication(req.user.email);
  }
