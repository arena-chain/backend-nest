...
export class AuthService {
  ...
  async updateProfile(email: string, updateData: { nickname?: string; country?: string; avatar?: string; region?: string }) {
    const user = await this.usersService.findByEmail(email);
    if (!user) throw new NotFoundException('User not found');
  ...
  async generateTwoFactorSecret(email: string) {
    ...
    await this.usersService.update(user._id.toString(), { twoFactorSecret: secret });

    const qrCode = await toDataURL(otpAuthUrl);
    return { secret, qrCode };
  }

  async turnOnTwoFactorAuthentication(email: string, token: string) {
    const user = await this.usersService.findByEmail(email);
    if (!user) throw new NotFoundException('User not found');
    ...
    await this.usersService.update(user._id.toString(), { isTwoFactorEnabled: true });
    return { message: '2FA enabled successfully' };
  }

  async disableTwoFactorAuthentication(email: string) {
    await this.updateByEmail(email, { isTwoFactorEnabled: false, twoFactorSecret: undefined });
