...
function buildBO3Result(t1: string, t2: string, win1: boolean, sweep: boolean) {
  ...
  if (sweep) {
    games.push({
      ...
      winnerId: winner,
      mapName: rMap(),
      team1Score: win1 ? 13 : 5,
      team2Score: win1 ? 5 : 13,
      durationMinutes: 35,
    });
    games.push({
      gameNumber: 2,
      winnerId: winner,
      mapName: rMap(),
      team1Score: win1 ? 13 : 7,
      team2Score: win1 ? 7 : 13,
      durationMinutes: 38,
    });
    return {
      games,
      team1GamesWon: win1 ? 2 : 0,
      team2GamesWon: win1 ? 0 : 2,
      winnerId: winner,
      loserId: loser,
    };
  }
  games.push({
    ...
    winnerId: winner,
    mapName: rMap(),
    team1Score: win1 ? 13 : 9,
    team2Score: win1 ? 9 : 13,
    durationMinutes: 42,
  });
  games.push({
    gameNumber: 2,
    winnerId: loser,
    mapName: rMap(),
    team1Score: win1 ? 7 : 13,
    team2Score: win1 ? 13 : 7,
    durationMinutes: 40,
  });
  games.push({
    gameNumber: 3,
    winnerId: winner,
    mapName: rMap(),
    team1Score: win1 ? 13 : 11,
    team2Score: win1 ? 11 : 13,
    durationMinutes: 45,
  });
  return {
    games,
    team1GamesWon: win1 ? 2 : 1,
    team2GamesWon: win1 ? 1 : 2,
    winnerId: winner,
    loserId: loser,
  };
}
...
const TEAM_DEFS = [
  {
    name: 'Team Phantom',
    org: 'Phantom Esports',
    mgr: { nick: 'PhantomGM', country: 'TN', first: 'Karim', last: 'Ben Ali' },
    pIdx: [0, 1, 2],
  },
  {
    name: 'Nexus Force',
    org: 'Nexus Gaming',
    mgr: { nick: 'NexusGM', country: 'FR', first: 'Jean', last: 'Dupont' },
    pIdx: [3, 4, 5],
  },
  {
    name: 'Eclipse Guild',
    org: 'Eclipse Organization',
    mgr: { nick: 'EclipseGM', country: 'PL', first: 'Adam', last: 'Kowalski' },
    pIdx: [6, 7, 8],
  },
  {
    name: 'Vortex United',
    org: 'Vortex Sports',
    mgr: { nick: 'VortexGM', country: 'DE', first: 'Klaus', last: 'Müller' },
    pIdx: [9, 10, 11],
  },
  {
    name: 'Blaze Collective',
    org: 'Blaze Esports',
    mgr: { nick: 'BlazeGM', country: 'ES', first: 'Carlos', last: 'García' },
    pIdx: [12, 13, 14],
  },
  {
    name: 'Cipher Protocol',
    org: 'Cipher Gaming',
    mgr: { nick: 'CipherGM', country: 'IT', first: 'Marco', last: 'Rossi' },
    pIdx: [15, 16, 17],
  },
];
...
async function seed() {
  ...
  for (const td of TEAM_DEFS) {
    const mgrUser = await UserModel.create({
      ...
      passwordHash: pwHash,
      nickname: td.mgr.nick,
      role: 'team_manager',
      country: td.mgr.country,
      region: 'EUROPE',
      isEmailVerified: true,
    });
    ...
    const mgrProfile = await TeamManagerProfileModel.create({
      ...
      team: team._id,
      organizationName: td.org,
      firstName: td.mgr.first,
      lastName: td.mgr.last,
      status: 'approved',
      isVerified: true,
    });
    await TeamModel.findByIdAndUpdate(team._id, {
      teamManager: mgrProfile._id,
    });
    teams.push(team);
  ...
  for (const s of schedule) {
    ...
    for (let i = 0; i < s.pairs.length; i++) {
      ...
      const startTime = new Date(
        round.startDate.getTime() + i * 2 * 3600 * 1000,
      );
  ...
  console.log(
    '   ✅ 15 matches (9 COMPLETED | 3 ONGOING round | 3 SCHEDULED)\n',
  );
  ...
  await BracketModel.create({
    ...
    slots: [
      ...
      },
      {
        ...
        team1Id: t2,
        team2Id: t4,
        matchId: grandFinal._id.toString(),
        status: 'READY',
      },
    ],
  });
  console.log(
    '   ✅ Playoff bracket ACTIVE — Grand Final: Nexus Force vs Vortex United\n',
  );
