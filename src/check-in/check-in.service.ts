...
export class CheckInService {
  ...
  async createForMatch(dto: CreateCheckInDto): Promise<CheckIn> {
    ...
    if (existing)
      throw new ConflictException('Check-in already exists for this match.');
  ...
  async checkIn(
    ...
    if (isTeam1) {
      if (record.team1CheckedIn)
        throw new ConflictException('Team 1 already checked in.');
      record.team1CheckedIn = true;
      record.team1CheckedInAt = new Date();
    } else {
      if (record.team2CheckedIn)
        throw new ConflictException('Team 2 already checked in.');
