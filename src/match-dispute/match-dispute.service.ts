...
export class MatchDisputeService {
  constructor(
    @InjectModel(MatchDispute.name)
    private readonly disputeModel: Model<MatchDisputeDocument>,
  ) {}
  ...
  async submit(dto: CreateMatchDisputeDto): Promise<MatchDispute> {
    ...
    if (existing) {
      throw new ConflictException(
        'An open dispute already exists for this match by your team.',
      );
    }
  ...
  async markUnderReview(id: string): Promise<MatchDispute> {
    ...
    if (dispute.status !== DisputeStatus.PENDING) {
      throw new BadRequestException(
        'Only PENDING disputes can be moved to UNDER_REVIEW.',
      );
    }
