...
export class TournementsController {
  ...
  create(
    @Body() createTournementDto: CreateTournementDto,
  ...
  update(
    @Param('id') id: string,
    @Body() updateTournementDto: UpdateTournementDto,
  ) {
    console.log(
      `[TournamentsController] Incoming PATCH for ${id}:`,
      updateTournementDto,
    );
  ...
  @ApiResponse({
    status: 400,
    description: 'Bad request - Registration closed or tournament full',
  })
  @ApiResponse({ status: 404, description: 'Tournament not found' })
  ...
  @ApiParam({
    name: 'phaseName',
    description: 'Phase name (e.g., QUARTERFINALS)',
  })
  ...
  @ApiResponse({
    status: 400,
    description: 'Bad request - Phase already exists',
  })
  @ApiResponse({ status: 404, description: 'Tournament not found' })
  addPhase(
    @Param('id') id: string,
  ...
  @Get(':id/teams')
  @ApiOperation({
    summary: 'Get all teams in a tournament with their full player roster',
  })
  @ApiParam({ name: 'id', description: 'Tournament ID' })
