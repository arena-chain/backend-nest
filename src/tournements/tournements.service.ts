...
export class TournementsService {
  ...
  async create(createTournementDto: CreateTournementDto): Promise<Tournament> {
    try {
      ...
      // Notify all admin users about the new tournament
      try {
        const allUsers = await this.usersService.findAll();
        const admins = allUsers.filter(u => u.role === 'admin');
        const organizerId = savedTournament.organizerId?.toString() ?? '';
        const tournamentId = (savedTournament as any)._id?.toString() ?? '';
        for (const admin of admins) {
          const adminId = (admin as any)._id?.toString();
          if (!adminId || adminId === organizerId) continue; // Skip self-notify if organizer is admin
          await this.notificationService.createForUser({
            userId: adminId,
            title: '🏆 New Tournament Created',
            message: `A new tournament "${savedTournament.name}" has been created and is awaiting review.`,
            type: NotificationType.SYSTEM,
            category: NotificationCategory.SYSTEM,
            link: `/admin/tournaments/${tournamentId}`,
            metadata: { tournamentId, tournamentName: savedTournament.name },
          });
        }
      } catch (notifErr) {
        console.error('[TournamentsService] Failed to notify admins on creation:', notifErr);
      }
  ...
  async update(id: string, updateTournementDto: UpdateTournementDto): Promise<Tournament> {
    console.log(`[TournamentsService] Updating tournament ${id}:`, updateTournementDto);
    if (!Types.ObjectId.isValid(id)) {
      throw new BadRequestException('Invalid tournament ID');
    }

    // Fetch existing tournament to detect status change
    const existing = await this.tournamentModel.findById(id).lean().exec();
    if (!existing) {
      throw new NotFoundException(`Tournament with ID ${id} not found`);
    }
    ...
    if (existing.organizerId && updateTournementDto.status && updateTournementDto.status !== existing.status) {
      let title = '';
      let message = '';
      let shouldNotify = false;

      if (updateTournementDto.status === TournamentStatus.BLOCKED) {
        title = '🔒 Tournament Blocked';
        message = `Your tournament "${existing.name}" has been blocked by an administrator. Please contact support for more information.`;
        shouldNotify = true;
      } else if (updateTournementDto.status === TournamentStatus.OPEN_REGISTRATION && existing.status === TournamentStatus.PENDING_APPROVAL) {
        title = '✅ Tournament Approved';
        message = `Your tournament "${existing.name}" has been approved! Registration is now open.`;
        shouldNotify = true;
      } else if (updateTournementDto.status === TournamentStatus.REJECTED && existing.status === TournamentStatus.PENDING_APPROVAL) {
        title = '❌ Tournament Rejected';
        message = `Your tournament request "${existing.name}" has been rejected.`;
        shouldNotify = true;
      }
      ...
      if (shouldNotify) {
        try {
          await this.notificationService.createForUser({
            userId: existing.organizerId.toString(),
            title,
            message,
            type: NotificationType.SYSTEM,
            category: NotificationCategory.SYSTEM,
            link: `/player/tournaments/${id}`,
            metadata: { tournamentId: id, tournamentName: existing.name },
          });
        } catch (notifErr) {
          // Non-fatal: log but don't fail the update
          console.error('[TournamentsService] Failed to send status notification:', notifErr);
        }
      }
    }
