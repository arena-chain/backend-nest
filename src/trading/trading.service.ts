...
export class TradingService {
  ...
  private async executeTrade(
    ...
    maker.status =
      maker.filledAmount === maker.amount ? 'FILLED' : 'PARTIALLY_FILLED';
    await maker.save();

    taker.filledAmount += amount;
    taker.status =
      taker.filledAmount === taker.amount ? 'FILLED' : 'PARTIALLY_FILLED';
    await taker.save();
    ...
    await this.assetModel.findByIdAndUpdate(maker.assetId, {
      lastPrice: price,
    });
  ...
  async getOrderBook(assetId: string) {
    const bids = await this.orderModel.aggregate([
      ...
      },
      {
        $group: {
          _id: '$price',
          totalAmount: { $sum: { $subtract: ['$amount', '$filledAmount'] } },
        },
      },
      { $sort: { _id: -1 } },
    ]);
