...
export class MissionService {
  constructor(
    ...
    private progressModel: Model<UserMissionProgressDocument>,
    @InjectModel(MissionEventLog.name)
    private eventLogModel: Model<MissionEventLogDocument>,
  ...
  async incrementProgressForCriteria(
    ...
    const alreadyProcessed = await this.hasProcessedEvent(
      userId,
      criteriaType,
      dedupeKey,
    );
  ...
  async onTrainingCompleted(
    userId: string,
    payload?: { game?: string; amount?: number },
  ) {
  ...
  async getActiveCount(userId: string): Promise<number> {
    ...
    for (const m of missions) {
      ...
      const prog = await this.progressModel
        .findOne({
          userId: new Types.ObjectId(userId),
  ...
  async seed() {
    ...
    if (count > 0)
      return { message: `Already seeded (${count} missions exist)` };
    ...
    const missions: CreateMissionDto[] = [
      ...
      // Weekly — Individual
      {
        ...
        description:
          'Play and complete 25 competitive matches across any supported game on Arena Chain.',
        type: 'weekly',
        scope: 'individual',
        game: 'all',
