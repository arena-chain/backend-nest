...
export class ReservationService {
  ...
  async create(
    ...
    // 2. Check Sales Start
    if (
      tournament.ticketSalesStart &&
      new Date() < new Date(tournament.ticketSalesStart)
    ) {
      throw new BadRequestException(
        `Ticket sales start on ${new Date(tournament.ticketSalesStart).toLocaleString()}`,
      );
    }
    ...
    if (currentSold + quantity > typeConfig.capacity) {
      throw new BadRequestException(
        `Not enough tickets available. Remaining: ${typeConfig.capacity - currentSold}`,
      );
    }
