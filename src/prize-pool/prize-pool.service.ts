...
export class PrizePoolService {
  constructor(
    @InjectModel(PrizePool.name)
    private readonly prizePoolModel: Model<PrizePoolDocument>,
  ) {}

  async create(dto: CreatePrizePoolDto): Promise<PrizePool> {
    const existing = await this.prizePoolModel.findOne({
      seasonId: dto.seasonId,
    });
    if (existing) {
      throw new ConflictException(
        'A prize pool already exists for this season.',
      );
    }
